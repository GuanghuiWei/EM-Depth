...
class KITTIDepthAnything(Dataset):
    ...
    def _get_frames(self, data_dir):
        ...
        #split_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "datasets", "kitti", "splits", "eigen", "test_files.txt")
        split_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "splits", "eigen_zhou", "train_files.txt")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/pathto/your/kitti_data')
    ...
    # model = AutoModelForDepthEstimation.from_pretrained(Depth_Anything_Path)
    # # model.save_pretrained(save_path)
    # model.to(device)
    # model.eval()
    # image_processor = AutoImageProcessor.from_pretrained(Depth_Anything_Path)
    # # image_processor.save_pretrained(save_path)
    
    #Depth_Anything_Path = '/path/to/your_DepthAnything/'
    model = AutoModelForDepthEstimation.from_pretrained("LiheYoung/depth-anything-large-hf") #"LiheYoung/depth-anything-large-hf"
    model.to(device)
    model.eval()
    image_processor = AutoImageProcessor.from_pretrained("LiheYoung/depth-anything-large-hf")  #"LiheYoung/depth-anything-large-hf"
