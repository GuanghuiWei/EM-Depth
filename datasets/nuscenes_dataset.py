...
from .mono_dataset_nus_new import MonoDataset
    ...
    def get_all_intrinsic(self):
        for file in self.filenames:
            ...
            if folder not in self.K:
                ...
                # with open(cam_path, 'r') as fh:
                #
                #     intrinsic_mat = json.load(fh).get('intrinsic_mat')
                #     if intrinsic_mat is not None:
                #         self.K[folder][:3, :3] = np.array(intrinsic_mat, dtype=np.float32)
                #     else:
                #         print(f"Warning: 'intrinsic_mat' not found in {cam_path}")
