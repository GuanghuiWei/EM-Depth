...
class KITTIPseudoDepthDataset(KITTIDataset):
    """KITTI dataset which uses the updated ground truth depth maps
    """

    def __init__(self, *args, **kwargs):
        super(KITTIPseudoDepthDataset, self).__init__(*args, **kwargs)
    ...
    def get_depth(self, folder, frame_index, side, do_flip):
        f_str = "{:010d}.png".format(frame_index)
        f_str_max = "{:010d}.txt".format(frame_index)

        depth_path = os.path.join(
            self.data_path,
            "pseudo_depths_with_max",
            folder,
            "image_0{}/data".format(self.side_map[side]),
            f_str)
        ...
        depth_gt = max_val * depth_gt.astype(np.float32) / 65535.0
