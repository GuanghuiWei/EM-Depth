...
class CityscapesPreprocessedDataset(MonoDataset):
    ...
    def get_colors(self, city, frame_name, side, do_flip):
        ...
        inputs[("color", -1, -1)] = color[:, :w]
        inputs[("color", 0, -1)] = color[:, w:2*w]
        inputs[("color", 1, -1)] = color[:, 2*w:]

        if do_flip:
            for key in inputs:
                # inputs[key] = inputs[key].transpose(pil.FLIP_LEFT_RIGHT)
                inputs[key] = A.augmentations.transforms.HorizontalFlip(p=1)(image=inputs[key])['image']
                inputs[key] = Image.fromarray(inputs[key])
