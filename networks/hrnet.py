...
class SEBlock(nn.Module):
    def __init__(self, channels, ratio=4):
        ...
        self.fc_layers = nn.Sequential(
            nn.Linear(in_features=channels, out_features=int(channels//ratio), bias=False),
            nn.ReLU(),
            nn.Linear(in_features=int(channels//ratio), out_features=channels, bias=False),
        )
...
class Bottleneck(nn.Module):  # 残差块的变体 1x1降低通道 3x3提取特征 1x1恢复通道
    expansion = 4
...
def _hrnet(arch, pretrained, progress, **kwargs):
    ...
    if pretrained:
        if arch == 'hrnet18':
            pth_path = "./pretrain_weights/HRNet_W18_C_ssld_pretrained.pth"
        if arch == 'hrnet48':
            pth_path = "./pretrain_weights/HRNet_W48_C_ssld_pretrained.pth"
        if arch == 'hrnet64':
            print('please check the pretrained weights')

        loaded_state_dict = torch.load(pth_path, map_location='cpu')
        model_state_dict = model.state_dict()

        # 计算匹配和不匹配的键
        matched_keys = set(model_state_dict.keys()) & set(loaded_state_dict.keys())
        unmatched_model_keys = set(model_state_dict.keys()) - set(loaded_state_dict.keys())  # 模型需要但预训练没有
        unmatched_pretrained_keys = set(loaded_state_dict.keys()) - set(model_state_dict.keys())  # 预训练有但模型不需要

        # 统计数量
        num_matched = len(matched_keys)
        num_unmatched_model = len(unmatched_model_keys)
        num_unmatched_pretrained = len(unmatched_pretrained_keys)

        # 打印统计信息
        print(f"\n权重匹配情况统计:")
        print(f"✅ 匹配的权重数量: {num_matched}")
        print(f"❌ 模型需要但预训练缺少的权重数量: {num_unmatched_model}")
        print(f"⚠️ 预训练中存在但模型不需要的权重数量: {num_unmatched_pretrained}")

        # 打印不匹配的键名（前10个）
        print("\n🔍 模型需要但预训练缺少的键名:")
        for k in list(unmatched_model_keys):
            print(f"  {k}")
...
def hrnet48(pretrained=True, progress=True, **kwargs):
    r"""HRNet-48 model
    """
    model = _hrnet('hrnet48', pretrained, progress, **kwargs)
    model.num_ch_enc = [64, 48, 96, 192, 384]
    return model
